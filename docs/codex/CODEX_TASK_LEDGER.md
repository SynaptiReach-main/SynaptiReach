...
| 25 | First-run onboarding | Partial | Polished multi-step `/onboarding` wizard with save/resume; real workspace/session/settings/billing-intent/provider/lead/staff writes; launch readiness scoring; schema compatibility repairs added; build passes; local onboarding page/API fail-closed smoke passed; Obsidian note/MOC updated | authenticated browser walkthrough, Stripe Checkout redirect verification, live provider key save verification, real CSV import test | 2026-05-27 |
