...
| 25 | First-run onboarding | Partial | Checkout return no longer saves Billing as the permanent step; auth/session redirects added; status Sign Out added; readiness labels updated; CRM-aware onboarding defaults expanded into `crm_settings.metadata`; build verification in progress | authenticated browser walkthrough, Stripe Checkout redirect verification, live provider key save verification, real CSV import test, production manual retest | 2026-05-28 |
| 26 | User CRM portal usability | Partial | Shared owner-focus panels added across all scoped dashboard pages; sidebar navigation grouped; intelligence cards now show clearer why/next-step blocks; build passes; local dashboard route smoke returned 200 | authenticated browser/mobile visual review and click-through of new panels/modals/sidebar grouping | 2026-05-27 |
...
Date: 2026-05-28
Model: Codex
Prompt/Goal: Continue onboarding stabilization in place, fix Stripe return loading/saved-step behavior, harden auth/workspace scoping, preserve completed CRM usability work, and expand CRM-aware onboarding fields.
Completed: Read required knowledge, architecture, and decisions docs before editing. Inspected onboarding page/status/API routes, billing checkout, onboarding server helper, dashboard layout redirect behavior, CRM dashboard data helper, CRM APIs/settings patterns, dashboard pages, intelligence helpers, workflow/notification/staff/billing/service-menu code references. Fixed Stripe checkout success handling so `/onboarding?checkout=success&session_id=...&step=billing` calls `/api/onboarding/stripe-session`, records checkout session visibility/pending webhook state, shows Billing for the fresh return, and does not post a generic save with `currentStep=billing`. Added recoverable Stripe status retry UI. Preserved saved-step behavior by saving the next step on Continue and preserving the current step on Save/Finish Later. Added unauthenticated redirects to `/signin?returnTo=...`, completed-user redirect to `/dashboard`, submitted-user redirect to `/onboarding/status` unless editing, and status-page Sign Out. Tightened subscription checkout workspace ownership so raw workspace/body params cannot start checkout for another user's workspace. Updated readiness labels to `Business Profile` and `Billing Set-Up`. Added CRM-aware onboarding fields for main customer type, categories/top services, common questions/objections, emergency rules, stale thresholds, follow-up timing, owner assignment, analytics goals/baselines, AI risk/topics/draft mode, calendar reminder timing, retargeting/review timing, and usage warning/draft-only automation preferences. Mapped new fields into `crm_settings.metadata` only; no schema changes and no fake records/states.
Skipped: No onboarding redesign, no User CRM usability rewrite, no OwnerFocusPanel/sidebar/dashboard route changes, no Stripe live mode, no fake paid/subscribed/trialing state, no fake users/leads/revenue/analytics, no auto-send, and no schema migration.
Partial: Authenticated browser/manual tests remain required for the production Stripe return, cross-user authorization, saved-step resume, and CRM field persistence.
Files changed: `app/onboarding/page.tsx`, `app/onboarding/status/page.tsx`, `app/api/onboarding/stripe-session/route.ts`, `app/api/billing/subscription/checkout/route.ts`, `app/(marketing)/signin/page.tsx`, `lib/onboarding/server.ts`, `tsconfig.json`, `docs/onboarding.md`, `docs/knowledge-map.md`, `docs/architecture.md`, `docs/decisions.md`, `docs/codex/CODEX_TASK_LEDGER.md`, `exports/FINAL_FULL_CRM_COMPLETION_CHECKLIST.md`.
Build result: `npm.cmd run build` passed and generated 153/153 static pages. First build attempt compiled/generated pages but timed out during trace finalization at 300 seconds; rerun with a longer timeout exited 0.
Tests run: Supabase/Stripe official-doc check; source inspection; `npx.cmd tsc --noEmit --pretty false` initially surfaced the TypeScript 6 `baseUrl` deprecation, so `tsconfig.json` now uses `ignoreDeprecations: "6.0"`. A follow-up `tsc --noEmit` still reports pre-existing broad type issues outside this pass, while `next build` skips type validation and passes.
Next recommended task: Run authenticated production/test-mode manual checks for Stripe return, exact saved-step resume, submitted status routing, approved dashboard routing, and CRM settings metadata persistence.

### 2026-05-27 - User CRM Portal Usability Pass

Date: 2026-05-27
Model: Codex
Prompt/Goal: Run a User CRM portal usability pass focused on clarity, navigation, spacing, hierarchy, and reducing overwhelm for non-technical business owners without redoing onboarding or billing.
Completed: Read `docs/codex/USER_CRM_PORTAL_USABILITY_GOAL.md`, `docs/codex/CODEX_TASK_LEDGER.md`, `exports/FINAL_FULL_CRM_COMPLETION_CHECKLIST.md`, `docs/knowledge-map.md`, `docs/architecture.md`, and `docs/decisions.md` before editing. Inspected `/dashboard`, `/dashboard/leads`, `/dashboard/pipeline`, `/dashboard/tasks`, `/dashboard/calendar`, `/dashboard/communications`, `/dashboard/marketing`, `/dashboard/workflow`, `/dashboard/analytics`, `/dashboard/ai_assistant`, `/dashboard/settings`, the shared dashboard layout, Settings history component, and shared intelligence panel. Added a shared `OwnerFocusPanel` and wired it into all scoped dashboard pages with real page counts/statuses where available and safe setup guidance where counts are not appropriate. Grouped dashboard sidebar navigation into Start, Customers, Growth, Work, and Review; renamed the sidebar workflow item to Automations while preserving `/dashboard/workflow`. Replaced static topbar readiness badges with non-fake safety/status language. Clarified the shared Business Intelligence panel so cards show explicit "Why it matters" and "Suggested next step" sections. Changed an AI Assistant source chip from underscored internal wording to customer-facing Built-in Intelligence.
Skipped: No onboarding redo, no billing/webhook/payment-history redo, no local AI redo, no CRM Intelligence data-flow changes, no simulation bootstrap changes, no public page changes, no route deletion, no fake records, no auto-send, no auto-charge, and no Stripe live-mode change.
Partial: Manual authenticated browser/mobile review remains required for visual density, responsive spacing, sidebar grouping, modal interactions, and click-through behavior.
Blocked: In-app Browser automation could not be used because tool discovery returned no Browser tools in this session.
Files changed: `components/dashboard/OwnerFocusPanel.tsx`, `components/intelligence/MiniBrainInsightPanel.tsx`, `app/dashboard/layout.tsx`, `app/dashboard/page.tsx`, `app/dashboard/leads/page.tsx`, `app/dashboard/pipeline/page.tsx`, `app/dashboard/tasks/page.tsx`, `app/dashboard/calendar/page.tsx`, `app/dashboard/communications/page.tsx`, `app/dashboard/marketing/page.tsx`, `app/dashboard/workflow/page.tsx`, `app/dashboard/analytics/page.tsx`, `app/dashboard/ai_assistant/page.tsx`, `app/dashboard/settings/page.tsx`, `docs/knowledge-map.md`, `docs/architecture.md`, `docs/decisions.md`, `docs/codex/CODEX_TASK_LEDGER.md`, `exports/FINAL_FULL_CRM_COMPLETION_CHECKLIST.md`, Obsidian `User CRM Portal Usability.md`, Obsidian `SynaptiReach MOC.md`
Build result: `npm.cmd run build` passed and generated 150/150 static pages.
Tests run: Required docs reads; source audit of all scoped dashboard pages/layout/shared panels; customer-facing wording scan for removed static/fake badges and underscored built-in intelligence label; `npm.cmd run build`; local production route smoke for `/dashboard`, `/dashboard/leads`, `/dashboard/pipeline`, `/dashboard/tasks`, `/dashboard/calendar`, `/dashboard/communications`, `/dashboard/marketing`, `/dashboard/workflow`, `/dashboard/analytics`, `/dashboard/ai_assistant`, and `/dashboard/settings`, all returning HTTP 200.
Next recommended task: Run an authenticated desktop/mobile browser pass for the new owner-focus panels, grouped sidebar, intelligence card detail modal, and each page's primary create/review actions.

### 2026-05-27 - First-run Onboarding Final Verification Pass

Date: 2026-05-27
Model: Codex
Prompt/Goal: Finish only the onboarding pass already in progress from the current working tree; do not start the User CRM usability pass.
Completed: Read `docs/codex/CODEX_TASK_LEDGER.md`, `docs/onboarding.md`, and `exports/FINAL_FULL_CRM_COMPLETION_CHECKLIST.md` first. Re-read applicable Supabase and Stripe guidance. Inspected all current onboarding changes in `app/onboarding`, `app/api/onboarding`, `lib/onboarding/server.ts`, `app/api/crm/leads/import/route.ts`, and `lib/crm/importLeadsCsv.ts`. Verified onboarding uses authenticated real writes through existing `workspaces`, `workspace_members`, `onboarding_sessions`, `crm_settings`, `crm_billing_accounts`, `crm_provider_connections`, `leads`, `crm_csv_imports`, and `crm_staff` structures. Verified onboarding stores plan/billing intent only and does not create paid/subscribed state. Verified provider keys are sanitized from session payloads, encrypted server-side before storage, omitted from readiness snapshots, and cleared from client state after save. Verified `.env.local` is not tracked. Verified CSV import remains compatible, still enforces empty/5000-row validation, now scopes duplicate checks to workspace when known, and records import history without breaking the existing CRM import route. Verified onboarding links to `/dashboard`, `/dashboard/settings`, and `/dashboard/leads`. Added idempotent schema repairs for older `workspace_members` and `onboarding_sessions` tables missing current onboarding metadata/status/updated_at columns. Tightened `/api/onboarding/integrations` to return fail-closed status codes from shared onboarding save. Updated `docs/onboarding.md` and Obsidian `SynaptiReach Onboarding.md`.
Skipped: No User CRM usability pass, no completed User CRM work redo, no Stripe live-mode switch, no `.env.local` commit, no auto-send, no auto-charge, no fake paid/subscribed state, and no fake CRM records.
Partial: Authenticated browser walkthrough and live Supabase row verification remain manual/session-dependent. Stripe Checkout redirect from onboarding Billing step still needs authenticated test-mode verification. Provider key save/readiness and real CSV import still need live verification.
Blocked: Signed Stripe subscription activation remains blocked on Stripe Checkout/webhook confirmation. Customer receipt email remains blocked until a real Resend sending domain is purchased and verified.
Files changed: `app/onboarding/page.tsx`, `app/onboarding/select/page.tsx`, `app/api/onboarding/save/route.ts`, `app/api/onboarding/complete/route.ts`, `app/api/onboarding/integrations/route.ts`, `app/api/onboarding/upload/route.ts`, `lib/onboarding/server.ts`, `app/api/crm/leads/import/route.ts`, `lib/crm/importLeadsCsv.ts`, `supabase/user_crm_full_completion_schema.sql`, `docs/onboarding.md`, `docs/knowledge-map.md`, `docs/architecture.md`, `docs/decisions.md`, `docs/codex/CODEX_TASK_LEDGER.md`, `exports/FINAL_FULL_CRM_COMPLETION_CHECKLIST.md`, Obsidian `SynaptiReach Onboarding.md`
Build result: `npm.cmd run build` passed and generated 150/150 static pages.
Tests run: Required docs reads; source review of onboarding real-write/state boundaries; `.env.local` tracked-file check; secret/billing/fake-data greps; schema compatibility inspection; `npm.cmd run build`; local production smoke for `/onboarding` and `/onboarding/select`; fail-closed probes for unauthenticated `GET /api/onboarding/save` returning 401, unauthenticated `POST /api/onboarding/integrations` returning 401, and `POST /api/onboarding/upload` returning setup-required 501.
Next recommended task: Run an authenticated browser onboarding walkthrough with a real user/session, verify Supabase rows, test Stripe Checkout handoff in test mode, and only then commit/clean onboarding before starting the User CRM usability pass.

### 2026-05-27 - First-run Onboarding Prompt Pack Pass

Date: 2026-05-27
Model: Codex
Prompt/Goal: Start the SynaptiReach Onboarding Prompt Pack from the current working tree without redoing completed User CRM work.
Completed: Read `docs/codex/CODEX_TASK_LEDGER.md`, `exports/FINAL_FULL_CRM_COMPLETION_CHECKLIST.md`, `docs/knowledge-map.md`, `docs/architecture.md`, and `docs/decisions.md`. Searched for onboarding prompt files in `docs/codex`, `docs/onboarding`, `docs/synaptireach-onboarding-prompts`, `exports`, and onboarding/prompt/master filenames. No dedicated onboarding prompt pack was present; used the onboarding instructions in `docs/codex_synaptireach_prompt_reading_roadmap.md` and `docs/codex_synaptireach_global_goal_runbook.md`. Replaced placeholder onboarding save/integration/upload behavior with authenticated real-state routes. Added shared onboarding server helper for workspace creation/resume, `workspace_members`, `onboarding_sessions`, `crm_settings`, `crm_billing_accounts` plan intent, encrypted `crm_provider_connections`, manual lead setup, CSV import, staff invite rows, and readiness scoring. Replaced legacy `/onboarding` with a polished 10-step save/resume wizard and updated `/onboarding/select`. Workspace-scoped CSV import now records `crm_csv_imports` and no longer checks duplicates globally when a workspace is known. Added `docs/onboarding.md`, updated architecture/knowledge/decision docs, and created/updated the Obsidian `[[SynaptiReach Onboarding]]` note plus MOC entry.
Skipped: No User CRM rebuild, no local AI redo, no Settings/Billing History redo, no public page redo, no Stripe live-mode switch, no `.env.local` commit, no auto-send, no auto-charge, no fake CRM records.
Partial: Authenticated browser walkthrough and live DB-write verification remain manual/session-dependent. Upload storage remains setup-required until real storage is connected. Billing setup records plan intent only unless the user starts Stripe Checkout.
Blocked: Customer receipt email remains blocked until a real Resend sending domain is purchased and verified. Full Stripe subscription activation still requires signed Checkout/webhook confirmation.
Files changed: `app/onboarding/page.tsx`, `app/onboarding/select/page.tsx`, `app/api/onboarding/save/route.ts`, `app/api/onboarding/complete/route.ts`, `app/api/onboarding/integrations/route.ts`, `app/api/onboarding/upload/route.ts`, `lib/onboarding/server.ts`, `app/api/crm/leads/import/route.ts`, `lib/crm/importLeadsCsv.ts`, `docs/onboarding.md`, `docs/knowledge-map.md`, `docs/architecture.md`, `docs/decisions.md`, `docs/codex/CODEX_TASK_LEDGER.md`, `exports/FINAL_FULL_CRM_COMPLETION_CHECKLIST.md`, Obsidian `SynaptiReach Onboarding.md`, Obsidian `SynaptiReach MOC.md`
Build result: `npm.cmd run build` passed after a longer timeout; generated 150/150 static pages. First build attempt compiled successfully but timed out while generating static pages at 180 seconds.
Tests run: Required docs reads; onboarding prompt discovery; `npm.cmd run build` twice; local production smoke for `/onboarding` and `/onboarding/select`; fail-closed probes for unauthenticated `GET /api/onboarding/save` returning 401 and `POST /api/onboarding/upload` returning setup-required 501.
Next recommended task: Run an authenticated browser onboarding walkthrough with a real user/session, verify the saved rows in Supabase, and test Stripe Checkout handoff from the Billing step in test mode.
