...
`crm_settings.metadata` is the CRM-population bridge for onboarding defaults: service areas/hours, main customer type, primary service/product categories, top services/products, manual service/product rows, common customer problems/questions/objections, emergency/priority rules, booking/quote/review notes, pipeline defaults, stale lead/deal thresholds, follow-up timing, owner assignment preferences, appointment types, reminder timing, task defaults, analytics goals/baselines, communication style, AI risk/draft preferences, calendar setup, marketing setup, retargeting/review-request timing, workflow draft preferences, and a `crm_setup_summary` for admin/onboarding review visibility. These values configure the real workspace and recommendations; onboarding does not generate fake CRM activity.
...
Stripe Checkout is the only card setup path. Checkout requests write `checkout_required`, `checkout_created`, or `pending_webhook` state and leave trial start/end timestamps empty until signed Stripe webhook events provide them. The checkout success return is processed through `/api/onboarding/stripe-session`, which records session visibility and pending webhook state without changing saved step progress or activating trial/subscription state. Email verification, Stripe card setup, and trial disclosures gate final onboarding completion. Dashboard layout redirects incomplete unsubmitted users back to onboarding, redirects submitted pending-review users to `/onboarding/status`, and allows completed users into the CRM.

Onboarding and onboarding status are auth/session protected in the client and fail closed in the APIs. Reads/writes resolve the authenticated Supabase user, then scope workspace/session access to that user's owned workspace. Raw query parameters cannot select another user's onboarding session.
