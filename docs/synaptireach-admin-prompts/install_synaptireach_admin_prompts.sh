...
# SynaptiReach Admin Portal Prompt Pack installer
# Run this from the root of your SynaptiReach project in Termux.

PROMPT_DIR="docs/synaptireach-admin-prompts"
mkdir -p "$PROMPT_DIR"

cat > "$PROMPT_DIR/00_master_admin_portal_prompt.md" <<'EOF'
...
cat > "$PROMPT_DIR/01_admin_foundation_layout.md" <<'EOF'
...
cat > "$PROMPT_DIR/02_user_management_impersonation.md" <<'EOF'
...
cat > "$PROMPT_DIR/03_admin_command_center.md" <<'EOF'
...
cat > "$PROMPT_DIR/04_staff_portal_permissions.md" <<'EOF'
...
cat > "$PROMPT_DIR/05_global_approval_denial.md" <<'EOF'
# Phase 5 - Global Approval and Denial System

Use the instructions from `00_master_admin_portal_prompt.md`.

## Goal

Build the SynaptiReach Global Approval and Denial System.

The approval system should handle all work that needs admin, staff, or client review.

## Approval Item Types

Support approval records for:
...
- Approval detail page/modal
- My approvals
- Client approvals
- Staff submitted work
- AI-generated approvals
- Revision requests
- Denied items archive

## Requirements

- Use real records only.
- No fake approvals.
- Add filters by client, staff, type, status, priority, due date.
- Every approval/denial/revision must be logged.
- Admin can approve/deny anything.
- Staff can only approve/deny if permitted.
- Client approval should be supported where applicable.
- Add clean empty states.
- Add confirmation for publish/launch actions.

## Deliverable

Deliver a global approval queue, item detail view, approval actions, revision workflow, filtering, and audit logging.

EOF

cat > "$PROMPT_DIR/06_service_fulfillment_engine.md" <<'EOF'
...
cat > "$PROMPT_DIR/07_admin_ai_minibrain.md" <<'EOF'
...
cat > "$PROMPT_DIR/08_client_finder_prospect_discovery.md" <<'EOF'
# Phase 8 - Client Finder and Prospect Discovery System

Use the instructions from `00_master_admin_portal_prompt.md`.

## Goal

Build the SynaptiReach Client Finder and Prospect Discovery System.

## Purpose

Help SynaptiReach discover, score, organize, and contact potential clients using real data sources and compliant outreach workflows.

## Prospect Weaknesses to Detect
...
Prepare for real integrations such as:
...
- Keep outreach status clear.
- Avoid unsafe automation that sends messages without approval.
- Require approval before launching outreach if sending integrations exist.

## Data Requirements

- Do not create fake prospects.
- Do not hardcode prospect examples.
- Use real data integrations when available.
- If integrations are not available, create the UI and service abstraction ready for real APIs.
- Manual prospect entry should be supported.
- CSV import should be supported if feasible.

## Deliverable

Deliver a real-data-ready Client Finder, prospect scoring interface, prospect detail page, outreach tools, staff assignment, and prospect-to-client conversion pipeline.

EOF

cat > "$PROMPT_DIR/09_revenue_proposals_packages.md" <<'EOF'
...
cat > "$PROMPT_DIR/10_templates_kb_reports_integrations_audit.md" <<'EOF'
# Phase 10 - Templates, Knowledge Base, Reports, Integrations, and Audit Logs

Use the instructions from `00_master_admin_portal_prompt.md`.

## Goal

Build the SynaptiReach internal systems for templates, knowledge base, reports, integrations, and audit logs.
...
Show real integration data for:
...
- Audit logs should be read-only except for owner-level retention/export controls.
- Templates and knowledge base should be searchable.
- Staff access should be permission-limited.
- Every sensitive log view should respect permissions.

## Deliverable

Deliver template library, knowledge base, reporting center, integrations health center, and audit log system.

EOF

cat > "$PROMPT_DIR/README.md" <<'EOF'
...
echo "✅ SynaptiReach Admin Portal prompt pack installed at: $PROMPT_DIR"
find "$PROMPT_DIR" -maxdepth 1 -type f | sort
